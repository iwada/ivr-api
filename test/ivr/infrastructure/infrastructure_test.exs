defmodule Ivr.InfrastructureTest do
  use Ivr.DataCase

  alias Ivr.Infrastructure

  describe "events" do
    alias Ivr.Infrastructure.Event
    ...
    test "list_events/0 returns all events" do
      event = event_fixture()
      assert Infrastructure.list_events() == [event]
    end

    test "get_event!/1 returns the event with given id" do
      event = event_fixture()
      assert Infrastructure.get_event!(event.id) == event
    end

    test "create_event/1 with valid data creates a event" do
      assert {:ok, %Event{} = event} = Infrastructure.create_event(@valid_attrs)
    ...
    test "create_event/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Infrastructure.create_event(@invalid_attrs)
    end

    test "update_event/2 with valid data updates the event" do
      event = event_fixture()
      assert {:ok, %Event{} = event} = Infrastructure.update_event(event, @update_attrs)
    ...
    test "update_event/2 with invalid data returns error changeset" do
      event = event_fixture()
      assert {:error, %Ecto.Changeset{}} = Infrastructure.update_event(event, @invalid_attrs)
      assert event == Infrastructure.get_event!(event.id)
    end

    test "delete_event/1 deletes the event" do
      event = event_fixture()
      assert {:ok, %Event{}} = Infrastructure.delete_event(event)
      assert_raise Ecto.NoResultsError, fn -> Infrastructure.get_event!(event.id) end
    end

    test "change_event/1 returns a event changeset" do
      event = event_fixture()
      assert %Ecto.Changeset{} = Infrastructure.change_event(event)
    end
  end
end
